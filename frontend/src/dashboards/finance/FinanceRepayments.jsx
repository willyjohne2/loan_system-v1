    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm font-bold text-slate-500 dark:text-slate-400 ">Consolidated collection log from all branches</p>
        </div>
          ...
          <Table
            ...
            renderRow={(row) => (
              <tr key={row.id} className="text-sm hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 last:border-0 transition-colors">
                ...
                <td className="px-4 py-4 text-slate-500 dark:text-slate-400 font-medium">{new Date(row.payment_date).toLocaleDateString()}</td>
...
export default FinanceRepayments;
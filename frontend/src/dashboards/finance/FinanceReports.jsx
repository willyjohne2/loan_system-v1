  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Audit and reconciliation tools</p>
      </div>
          ...
          <button 
            ...
            className={`p-6 rounded-xl border transition-all text-left flex flex-col gap-4 group ${
              activeReport === report.id 
                ...
                : 'bg-white dark:bg-slate-900 border-slate-200 hover:border-primary-300'
            }`}
          >
            <div className={`p-3 rounded-lg w-fit transition-colors ${
              activeReport === report.id 
                ? 'bg-primary-600 text-white' 
                : 'bg-slate-100 text-slate-500 dark:text-slate-400 group-hover:bg-primary-100 group-hover:text-primary-600'
            }`}>
              <report.icon className="w-6 h-6" />
            </div>
            <div>
              <p className={`font-bold ${activeReport === report.id ? 'text-primary-900' : 'text-slate-900 dark:text-white '}`}>
                {report.label}
              </p>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">View financial details</p>
            </div>
          </button>
        ))}
      </div>
      ...
      {activeReport && (
        <Card className="p-0 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 dark:bg-slate-800 flex justify-between items-center">
            <h3 className="font-bold text-slate-900 dark:text-white uppercase tracking-wider text-xs">{activeReport.replace('_', ' ')}</h3>
          ...
          {activeReport === 'aging_report' ? (
            <Table
              ...
              renderRow={(row, i) => (
                <tr key={i} className="hover:bg-slate-50 dark:bg-slate-800 ">
                  ...
                  <td className="px-6 py-4 font-bold text-slate-900 dark:text-white ">{formatKES(row.balance)}</td>
            ...
            <Table
              ...
              renderRow={(row, i) => (
                <tr key={i} className="hover:bg-slate-50 dark:bg-slate-800 ">
                  <td className="px-6 py-4 text-slate-500 dark:text-slate-400 ">{new Date(row.date).toLocaleDateString()}</td>
                  <td className="px-6 py-4 font-medium text-slate-900 dark:text-white ">{row.customer}</td>
                  ...
                  <td className="px-6 py-4 text-slate-500 dark:text-slate-400 ">{row.method}</td>
                </tr>
              )}
            />
          ) : (
            <Table
              ...
              renderRow={(row, i) => (
                <tr key={i} className="hover:bg-slate-50 dark:bg-slate-800 ">
                  <td className="px-6 py-4 font-medium text-slate-900 dark:text-white ">{row.item}</td>
                  ...
                  <td className="px-6 py-4 font-bold text-slate-900 dark:text-white font-mono">{formatKES(row.balance)}</td>
                </tr>
              )}
            />
          )}
          ...
          {reportData.length === 0 && (
            <div className="p-20 text-center flex flex-col items-center gap-4">
              ...
              <div>
                <p className="font-bold text-slate-900 dark:text-white ">No report data found</p>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Try refreshing or adjusting your search criteria</p>
              </div>
            </div>
          )}
        </Card>
